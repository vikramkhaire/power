...
for zbin in zbinarr:
    ...
    for waverange in waverangearr:
        outpath = '/home/vikram/output_power/hsla_power/high_SN'
        Power_table = tbl.Table(
            ...
            names=['LS K-modes', 'LS Power(noise and wind corr)', 'N_used', 'C', 'z', 'noise_power'])
        Power_table.add_column(tbl.Column(np.sqrt(np.diag(Power_table['C'])), name='sigma'))
        out = Power_table['z', 'LS K-modes', 'LS Power(noise and wind corr)', 'sigma', 'N_used', 'noise_power']
...
for zbin in zbinarr:
    ...
    for waverange in waverangearr:
        outpath = '/home/vikram/output_power/hsla_power/high_SN'
        Power_table = tbl.Table(
            compute_power_hsla(data_path=data_path, use_metalmasking=True, zbin=zbin, wavelim=waverange, fill_with_noise = True),
            names=['LS K-modes', 'LS Power(noise and wind corr)', 'N_used', 'C', 'z', 'noise_power'])
        Power_table.add_column(tbl.Column(np.sqrt(np.diag(Power_table['C'])), name='sigma'))
        out = Power_table['z', 'LS K-modes', 'LS Power(noise and wind corr)', 'sigma', 'N_used', 'noise_power']
        out.write(outpath + 'Power_gaps_filled_z{:.2f}-{:.2f}_wave{}-{}.txt'.format(zbin[0], zbin[1], waverange[0],
                                                                                        waverange[1]),
                  format="ascii.commented_header", overwrite=True)
